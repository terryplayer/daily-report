...
def get_model_summary(history, rs_results=None, sector_rotation=None):
    ...
    for sec, scores in rs_by_sector.items():
        rs_scores[sec] = round(sum(scores) / len(scores), 1) if scores else 50
    ...
    for sec, scores in mf_by_sector.items():
        mf_scores[sec] = round(sum(scores) / len(scores), 1) if scores else 50
    ...
    weights = {'rs': 0.25, 'momentum': 0.20, 'mrd': 0.10, 'multi_factor': 0.30, 'rotation': 0.15}
...
def format_model_summary_text(model_summary):
    ...
    lines.append(f'{"板块":<10}{"综合":>5}{"RS":>5}{"动量":>5}{"MRD":>5}{"多因子":>5}{"轮动":>5}{"配置":>6}')
    ...
    for sec, info in sorted(model_summary['composite'].items()):
        ...
        action = info['action']
        rs_v = ms.get('rs', '-')
        mom_v = ms.get('momentum', '-')
        mrd_v = ms.get('mrd', '-')
        mf_v = ms.get('multi_factor', '-')
        rot_v = ms.get('rotation', '-')
        lines.append(f'{sec:<10}{score:>5.0f}{rs_v:>5}{mom_v:>5}{mrd_v:>5}{mf_v:>5}{rot_v:>5}{action:>6}')
